package  
{
	...
	public class main extends Sprite 
	{
		...
		private var movie_id:String = loaderInfo.parameters['movie_id'];
		private var live:String = loaderInfo.parameters['live'];
		private var my_user_id:String = loaderInfo.parameters['user_id'];
		private var my_role:String = loaderInfo.parameters['role'];
		
		// elapsed time(sec) when accessed if live
		...
		private var play:Loader = new Loader();
		...
		public function main() 
		{
			...
			trace("User_ID = " + my_user_id + " Role = " + my_role + " Movie ID = " + movie_id + " Live flag = " + live);
			...
			play.load(new URLRequest("http://" + webServerAddress + "/img/Play.png"));
			...
			// create play button
			start = new SimpleButton(play,play,play,play);
			start.x = vdo.x + 157;
			start.y = vdo.y + 138;
		...
		function sendComment():void {
			...
			// user_id
			// role
			var from:String = "subscribe";
				
			// <message>
			var time:Number = elapsedTime + ns.time;
			var value:String = comment.text;
			var type:String = commentType.selectedItem.data;
			var target:String = "stored";
		...
		function onEnterFrame(e:Event) {
			timeLabel.text = _timeFormat(elapsedTime + ns.time);
			commentStream();
		}
		...
		function commentStream() {
			for (var i in comments) {
				for (var j in comments[i]) {
					comments[i][j].x = (( i / 10) - (elapsedTime + ns.time)) * 550 / 4;
				}
			}
		}
		...
		function receive_data(event:DataEvent):void {
			...
			// calculate elapsed time if it is live streaming
			if (live == "1" && elapsedTime == 0)
				elapsedTime = Number(data.elapsedTime) / 1000;
